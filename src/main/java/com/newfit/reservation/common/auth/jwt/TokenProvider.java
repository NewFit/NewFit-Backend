...
public class TokenProvider {    // JWT의 생성 및 검증 로직 담당 클래스
	...
	public Authentication getAuthentication(String token, HttpServletRequest request) {
		...
		Set<SimpleGrantedAuthority> authorities = Collections.singleton(
			new SimpleGrantedAuthority(authority.getRoleType().getDescription()));
	...
	public Authentication getAnonymousAuthentication(String token) {
		Claims claims = getClaims(token);
		Set<SimpleGrantedAuthority> authorities = Collections.singleton(
			new SimpleGrantedAuthority(RoleType.GUEST.getDescription()));
		...
		if (claims.getSubject() != null) {
			...
			return new UsernamePasswordAuthenticationToken(
				new org.springframework.security.core.userdetails.User("anonymous", "", authorities), token,
				authorities);
		}
	}
	...
	public Authentication getAdminAuthentication() {
		Set<SimpleGrantedAuthority> authorities = Collections.singleton(
			new SimpleGrantedAuthority(RoleType.ADMIN.getDescription()));

		return new UsernamePasswordAuthenticationToken(
			new org.springframework.security.core.userdetails.User("admin", "", authorities), null, authorities);
	}
