...
public class CreditService {
	...
	public UserRankInfoListResponse getRankInGym(Long authorityId) {
		...
		List<CreditRanking> creditList = creditRepository
			.findTop10ByGymIdAndYearAndMonth(authority.getGym().getId(), (short)now.getYear(),
	...
	public void checkConditionAndAddCredit(Reservation reservation, Authority authority,
		LocalDateTime endEquipmentUseAt) {
		LocalDateTime now = now();

		if (checkConditions(reservation, endEquipmentUseAt)) {
			if (authority.getCreditAcquisitionCount() != 10) {
				Credit credit = creditRepository.findByAuthorityAndYearAndMonth(authority, (short)now.getYear(),
						(short)now.getMonthValue())
	...
	private boolean isStartTagInTime(Reservation reservation) {
		return (reservation.getStartTagAt().isBefore(reservation.getStartAt().plusMinutes(5))
			&& reservation.getStartTagAt().isAfter(reservation.getStartAt()))
