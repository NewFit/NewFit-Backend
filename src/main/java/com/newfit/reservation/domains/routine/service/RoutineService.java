...
public class RoutineService {
	 ...
	 */
	public void updateRoutineName(Long routineId, UpdateRoutineNameRequest request) {
		Routine routine = findById(routineId);
		validateDuplicate(routine.getAuthority(), request.getRoutineName());
		routine.updateName(request.getRoutineName());
	}
	...
	// 루틴의 특정 기구를 예약
	private RoutineReservationResponse reserveOneInRoutine(Long authorityId, Long equipmentId, LocalDateTime startAt,
		LocalDateTime endAt) {
		...
		while (attempt != 5) {
			try {
				EquipmentGym equipmentGym = equipmentGymRepository.findAvailableByEquipmentIdAndStartAtAndEndAt(
